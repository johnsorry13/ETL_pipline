...
class UniversalParser(BaseParser):
    ...
    def load_urls(self):
        ...
        try:
            ...
            self.logger.error(f"Ошибка загрузки списка URL {exec}")
            raise


    def fetch(self, url: str):
        self.logger.info(f"Загружаю прокси для {url}")
        try:
            proxy = self._proxy_manager.get_proxy()
            self.logger.info(f"Прокси {proxy['host']} успешно загружен для {url}")
        except Exception as exec:
            self.logger.error(f"Ошибка загрузки прокси для {url}")
            raise
        ...
        self.logger.info(f"Загрузка страницы: {url}, прокси: {proxy['host']}")
        try:
            html = requests.get(url, proxies=proxies, timeout=10)
            html.raise_for_status()
            self.logger.info(f"Страница: {url} успешно загружена, прокси: {proxy['host']}")
        except Exception as exec:
            self.logger.error(f"Ошибка загрузки страницы: {url}, прокси: {proxy['host']}")
            raise
        return html, proxy
    ...
    def parse(self, html, proxy, url):
        ...
        item = {'название': name, 'цена': price, 'date': timestamp, 'proxy': proxy['host'], 'url': url}
        return item
        ...
        try:
            with open(f'{self._store_config["shop"]}.csv', 'a', encoding='utf-8-sig', newline='') as f:
                ...
                for res in self.streaming_result():
                    if "ошибка" in res:
                        writer.writerow(["", "", "", "", f"{res['ошибка']}", f"{res['url']}"])

                    else:
                        writer.writerow([res['название'], res['цена'], res['date'], res['proxy'], "", res['url']])
